...
## Fluxo de Decapsulamento de Pacotes
```mermaid
graph TD
    A[Placa de Rede / Socket] --> B(Ethernet)
    B -- eth.proto == 1544 --> C[ARP Decoder]
    B -- eth.proto == 8 --> D(IPv4)
    D -- ipv4.proto == 1 --> E[ICMP]
    D -- ipv4.proto == 6 --> F(TCP)
    D -- ipv4.proto == 17 --> G(UDP)
    F -- Porta 80 --> H[HTTP Decoder]
    G -- Porta 53 --> I[DNS Decoder]
```
---
## 📋 Pré-requisitos
